...
// for decides is for comment or post
func (p *Post) PostType(i int) string {
	if i == 1 {
		return "post"
	} else {
		return "comment"
...
type UserRepo interface {
	...
	ListPost(*ListQuery, *Pagination) ([]Post, error)
	ViewPost(int) (*Post, error)
	ListPrivatePost(*ListQuery, *Pagination) ([]Post, error)
	ListPostCommentsCount([]int) ([]CommentCount, error)
	UpdatePost(*Post) (*Post, error)
	DeletePost(*Post) error

	CreateComment(*Post) (*Post, error)
	ListPostComments(*ListQuery, *Pagination, int) ([]Post, error)
	ViewPostComment(int) (*Post, error)
	UpdatePostComment(*Post) (*Post, error)
	DeletePostComment(*Post) error
}
